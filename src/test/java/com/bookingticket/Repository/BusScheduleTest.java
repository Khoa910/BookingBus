...
public class BusScheduleTest {
    ...
    public void testAddBusSchedule() {
        // Giả sử bạn đã có các đối tượng Bus, BusStation từ cơ sở dữ liệu hoặc Mock
        // Lấy đối tượng Bus từ cơ sở dữ liệu (ví dụ bằng repository)
        Bus bus = busRepository.findById(1L).orElseThrow(() -> new RuntimeException("Bus not found"));

        BusStation departureStation = busStationRepository.findById(1L).orElseThrow(() -> new RuntimeException("Departure station not found"));
        BusStation arrivalStation = busStationRepository.findById(2L).orElseThrow(() -> new RuntimeException("Arrival station not found"));
